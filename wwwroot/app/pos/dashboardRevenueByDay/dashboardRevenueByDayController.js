'use strict';
app.register.controller('dashboardRevenueByDayController', ['$scope', '$timeout', function ($scope, $timeout) {
    ...
    $scope.$on('$routeChangeSuccess', function () {
        init('dashboardRevenueByDay', $scope, true);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $scope.setting.valuelist.revenueByDayFilter = $scope.setting.valuelist.dashboardTimeMark.filter(x => x.id != "1" && x.id !="12");
            },
        });

        $scope.revenueByDayFilter = "2";
    ...
    $scope.loadData = function (listStore) {
        ...
        $scope.processDate($scope.revenueByDayFilter);
        $scope.loadchart();
    };
    ...
    $scope.loadchart = function () {
        ...
        $scope.postData("api/pos/DashboardPOS/getRevenueByDay", params, function (data) {
            if (data) {
                var labels = [];
                var value = [];
       ...
       // $timeout(function () {
            var ctx = document.getElementById("chartRevenueByDay");
