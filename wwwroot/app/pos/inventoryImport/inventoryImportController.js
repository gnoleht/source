'use strict';
app.register.controller('inventoryImportController', ['$scope', '$timeout', function ($scope, $timeout) {
    ...
    $scope.$on('$routeChangeSuccess', function () {
        ...
        $(window).resize(function () {
            var width = $("#inventoryImport .white_box").width();
            var height = $("#inventoryImport .white_box").height();
            var box_control = $("#inventoryImport .box_filter").height();

            if ($("#inventoryImport .box_filter").is(":visible")) {
                $('#inventoryImport .content_4 dd').height(height - 55 - box_control);
                $('#inventoryImport .content_6 .wrapper dd').height(height - 250 - box_control);
            }
            else {
                $('#inventoryImport .content_4 dd').height(height - 30);
                $('#inventoryImport .content_6 .wrapper dd').height(height - 240);
            }
    ...
    $scope.cmbStoreId = {
        ...
        allowClear: true,
        placeholder: inventoryImportTranslation.STOREID,
    };
    ...
    $scope.save = function () {
        ...
        $scope.postData('api/pos/InventoryImport/save', parameter, function (data) {
            if ($scope.action == "add") {
                $scope.loadData($scope.totalPage);
            }
            else {
                //var indexTemp = $scope.listData.findIndex(x => x.invImport.id == data.id);
                //$scope.listData[indexTemp].invImport = data;
    ...
    $scope.deleteData = function () {
        $scope.postData("api/pos/InventoryImport/Delete", { data: $scope.data }, function (result) {
            if (!result) {
                $scope.loadData();
            }
            else {
    ...
    $scope.loadData = function (page, pageEnd) {
        ...
        $scope.postData("api/pos/InventoryImport/get", params, function (object) {
            $scope.$applyAsync(function () {
                ...
                $scope.defaultFirstData = $scope.listData.length > 0 ? $scope.listData[0].invImport : null;
    ...
    $scope.loadObject = function (type, id) {
        ...
        $scope.postData("api/pos/InventoryImport/GetObject", params, function (data) {
            if (data) {
                if (type == "3") {
    ...
    $scope.loadGrid = function (id) {
        $scope.postData("api/pos/InventoryImport/GetDetail", { id: id }, function (data) {
    ...
    $scope.cmbFieldFilter = {
        ...
        allowClear: true,
        placeholder: inventoryImportTranslation.FIELDID,
    };
