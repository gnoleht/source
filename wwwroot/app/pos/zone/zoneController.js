'use strict';
app.register.controller('zoneController', ['$scope', function ($scope) {
    $scope.searchValue = "";
    ...
    $scope.$on('$routeChangeSuccess', function () {
        ...
        $(window).resize(function () {
            var width = $("#zone .white_box").width();
            var height = $("#zone .white_box").height();
            var box_control = $("#zone .box_filter").height();

            if ($("#zone .box_filter").is(":visible")) {
                $('#zone dd').height(height - 50 - box_control);
            }
            else {
                $('#zone dd').height(height - 25);
            }
    ...
    $scope.loadData = function () {
        $scope.postData("api/pos/zone/get", { searchValue: $scope.searchValue }, function (data) {
    ...
    $scope.save = function () {
        ...
        if ($scope.action == 'add') {
            $scope.postData("api/pos/zone/Add", { data: $scope.data }, function (data) {
                $scope.refreshFrm();
            ...
            $scope.postData("api/pos/zone/Update", { data: $scope.data }, function (data) {
                $scope.refreshFrm();
                $scope.defaultData = null;
                $('#modal-detail').modal('hide');
    ...
    $scope.deleteData = function () {
        $scope.post("api/pos/zone/remove", JSON.stringify($scope.data), function (lstDelete) {
            if (lstDelete) {
    ...
    $scope.changeActive = function (e, args) {
        ...
        if (item) {
            ...
            $scope.postData("api/pos/zone/update", { data: JSON.stringify(item) }, function (data) {
