'use strict';
app.register.controller('dashboardCostPercentRevenueController', ['$scope', '$timeout', function ($scope, $timeout) {
    ...
    $scope.$on('$routeChangeSuccess', function () {
        init('dashboardCostPercentRevenue', $scope, true);
        //debugger
        $.ajax({
            ...
            success: function (data) {
                ...
                $scope.setting.valuelist.costPercentRevenueFilter = $scope.setting.valuelist.dashboardTimeMark.filter(x => x.id != "1" && x.id != "12");
            },
        });

        $scope.costPercentRevenueFilter = "3";
    ...
    $scope.loadData = function (listStore) {
        ...
        $scope.processDate($scope.costPercentRevenueFilter);
        $scope.loadchart();
    };
    ...
    $scope.loadchart = function () {
        ...
        $scope.postData("api/pos/DashboardPOS/GetCostPercentRevenue", params, function (data) {
            if (data) {
                var labels = [];
                var value = [];
        ...
        $timeout(function () {
            var ctx = document.getElementById("costPercentRevenue");
