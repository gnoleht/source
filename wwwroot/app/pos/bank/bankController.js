'use strict';
app.register.controller('bankController', ['$scope', function ($scope) {
    ...
    $scope.loadData = function () {
        $scope.postData("api/pos/bank/get", { searchValue: $scope.searchValue }, function (data) {
    ...
    $scope.edit = function () {
        ...
        if (data == null)
            showError(bankTranslation["ERR_UPDATE_NULL"]);
        else {
            $scope.action = 'edit';
    ...
    $scope.save = function () {
        ...
        if ($scope.action == 'add') {
            $scope.postData("api/pos/bank/Add", { data: $scope.data }, function (data) {
                $scope.refreshFrm();
            ...
            $scope.postData("api/pos/bank/Update", { data: $scope.data }, function (data) {
                $scope.refreshFrm();
                $scope.defaultData = null;
                $('#modal-detail').modal('hide');
    ...
    $scope.deleteData = function () {
        $scope.post("api/pos/bank/remove", JSON.stringify($scope.data), function (lstDelete) {
            if (lstDelete) {
