'use strict';
app.register.controller('bankCardTypeController', ['$scope', function ($scope) {
    $scope.searchValue = "";
    ...
    $scope.loadData = function () {        
        $scope.postData("api/pos/bankCardType/get", { searchValue: $scope.searchValue }, function (data) {
    ...
    $scope.edit = function () {
        ...
        if (data == null)
            showError(bankCardTypeTranslation["ERR_UPDATE_NULL"]);
        else {
            $scope.action = 'edit';
    ...
    $scope.save = function () {
        ...
        if ($scope.action == 'add') {
            $scope.postData("api/pos/bankCardType/Add", { data: $scope.data }, function (data) {
                $scope.refreshFrm();
            ...
            $scope.postData("api/pos/bankCardType/Update", { data: $scope.data }, function (data) {
                $scope.refreshFrm();
                $scope.defaultData = null;
                $('#modal-detail').modal('hide');
    ...
    $scope.deleteData = function () {
        $scope.post("api/pos/bankCardType/remove", JSON.stringify($scope.data), function (lstDelete) {
            if (lstDelete) {
