'use strict';
app.register.controller('otherAddController', ['$scope', '$location', 'authService', function ($scope, $location, authService) {
    ...
    $scope.$on('$routeChangeSuccess', function () {
        ...
        $(window).resize(function () {
            var width = $("#otherAdd .white_box").width();
            //$("#vendorAdd .white_box").width(width);
            var height = $("#otherAdd .white_box").height();
            //$("#otherAdd .white_box").height(height);
            var box_control = $("#otherAdd .box_control").height();
            $("#otherAdd .scroll").height(height - box_control - 20);
        });
        $(window).resize();
