'use strict';
app.register.controller('bankController', ['$scope', '$location', 'authService', function ($scope, $location, authService) {
    ...
    $scope.$on('$routeChangeSuccess', function () {
        ...
        $(window).resize(function () {
            var width = $("#bank .white_box").width();
            //$("#vendorAdd .white_box").width(width);
            var height = $("#bank .white_box").height();
            //$("#vendorAdd .white_box").height(height);
            var box_control = $("#bank .box_control").height();
            $("#bank .scroll").height(height - box_control - 20);
        });
        $(window).resize();
