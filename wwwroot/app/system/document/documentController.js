...
app.register.controller('documentController', ['$scope', '$location', 'authService', function ($scope, $location, authService) {
    $scope.$on('$routeChangeSuccess', function () {
        ...
        $scope.parentFolderChange = function () {
            ...
            if ($scope.folder) {
                var parentFolder = $scope.grid.getItemById($scope.folder.parent);
                if (parentFolder) {
                    $scope.folder.module = parentFolder.module;
                    $scope.folder.productId = parentFolder.productId;
                    $scope.folder.projectId = parentFolder.projectId;

                    $scope.folder.viewRole = parentFolder.viewRole ? parentFolder.viewRole : [];
                    $scope.folder.editRole = parentFolder.editRole ? parentFolder.editRole : [];
                    $scope.folder.deleteRole = parentFolder.deleteRole ? parentFolder.deleteRole : [];
                    $scope.folder.approveRole = parentFolder.approveRole ? parentFolder.approveRole : [];
                }
            }
        };
        ...
        if ($scope.grid != null) {
            ...
            $scope.grid.onSelectedRowsChanged.subscribe(function (e, args) {
                var folder = $scope.grid.dataView.getItem(args.rows[0]);
    ...
    $scope.loadFolder = function (isReload) {
        var url = 'api/sys/document/getFolder?&isAdmin=' + $scope.isAdmin;
        if ($scope.params.pid)
            url += "&pid=" + $scope.params.pid;
