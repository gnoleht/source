'use strict';
app.register.controller('importSettingController', ['$scope', function ($scope) {
    $scope.$on('$routeChangeSuccess', function () {
        init('importSetting', $scope, true, false);
    });

    $scope.initImportSetting = function (grid) {
        $scope.data = {};
    ...
    $scope.initJS = function () {
        $("#importSetting #imgDropzone").dropzone({
            addedfile: function (file) {
                ...
                $("#importSetting #imgAvatar").attr("src", imageUrl);
            }
        });
    };
}]);
