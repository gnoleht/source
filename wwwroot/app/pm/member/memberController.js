'use strict';
app.register.controller('memberController', ['$scope', '$location', '$sce', 'authService', '$timeout', function ($scope, $location, $sce, authService, $timeout) {
    //
    ...
    $scope.$on('$routeChangeSuccess', function () {
        ...
        memberSetting.options.scope = $scope;

        $scope.refresh = function () {
            var url = memberSetting.grid.url + "?pjid=" + $scope.params.pjid + "&pid=" + $scope.params.pid + "&module=" + $scope.params.module;
    ...
    $(document).click(function (e) {
        var classList = e.target.className ? e.target.className.split(' ') : [];
        if (!e.target.className || classList.indexOf("showRoles") == -1) {
            $("#contextListCurrentRole").hide();
        }
        if (!e.target.className || classList.indexOf("showAreas") == -1) {
            $("#contextListCurrentArea").hide();
        }
    });
    ...
    $scope.add = function () {
        ...
        callModal('modal-detail', true, 'userId');
    ...
    $scope.edit = function () {
        ...
        //if checkRole == false:  không có quyền
    ...
    $scope.save = function () {
        ...
        if ($scope.action == 'add') {            
            ...
            $scope.postFile("api/pm/member/Update", $scope.frmFile, function (data) {
                ...
                //$scope.scope.dataView.updateItem(data.id, data);
                $scope.grid.invalidate();
            });
