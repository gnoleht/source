'use strict';
app.register.controller('reportCustomerController', ['$scope', '$timeout', function ($scope, $timeout) {
    ...
    $scope.$on('$routeChangeSuccess', function () {
        ...
        $timeout(function () {
            $scope.timeMark = "1";
            $scope.data.reportDetail= "6";
            $scope.data.statisticCustomer = "1";
            $scope.data.groupReport_Cust = "1";
        ...
        $(window).resize(function () {
            var width = $("#reportCustomer .white_box").width();
            var height = $("#reportCustomer .white_box").height();

            $('#reportCustomer #grvReportCustomer').height(height - 75);
    ...
    $scope.loadData = function () {
        var params = {
            statisticBy: $scope.data.statisticCustomer,
            groupLable: $scope.data.groupReport_Cust,
        ...
        $scope.postData("api/pos/reportCustomer/get", params, function (data) {
            if (data) {
                debugger
                $.each(data, function (index, item) {
                    if ($scope.data.groupReport_Cust == "1")
                        item.group = moment(item.createdTime).format("DD/MM/YYYY");
                    if ($scope.data.groupReport_Cust == "2")
                        item.group = moment(item.createdTime).format("MM/YYYY");
                    if ($scope.data.groupReport_Cust == "3")
                        item.group = moment(item.createdTime).format("Q");
                    if ($scope.data.groupReport_Cust == "4")
                        item.group = moment(item.createdTime).format("YYYY");
                    if ($scope.data.groupReport_Cust == "5")
                        item.group = item.cashierId
                    if ($scope.data.groupReport_Cust == "6")
    ...
    $scope.changeStatistic = function () {
        if ($scope.data.statisticCustomer == "1") {
            $scope.setting.temp = "1"
        }
        if ($scope.data.statisticCustomer == "2") {
            $scope.setting.temp = "2"
        }
        if ($scope.data.statisticCustomer == "3") {
